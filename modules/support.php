<div class="block-header-main hidden-xs">
        <div class="block-slide">
        	<?  
            $gal = $conn->query("select * from $tbl_gallery where cat=7 and display=1 and visibility=1 order by id desc");
            if($gal->num_rows > 0){
    ...
    <div class="row">
    	<div class="col-xs-12 project-right">
			<div class="block-home-title" style="border-bottom: none;">
				<h1 class="h1-project" style="color: #2c2e83;border-bottom: none;"><?=htmlspecialchars_decode($get->page('ho_tro','name'))?></h1>
			</div>
            ...
            ?>
				<?=htmlspecialchars_decode($get->page('ho_tro'))?>
            <?}?>
			</div>
    	</div>
    </div>
