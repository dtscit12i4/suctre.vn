<?php
    ...
    if(isset($_POST['tik'])) $tik = $_POST['tik'];else $tik='';
	if ($func == "del")
	{
		for ($i = 0; $i < count($tik); $i++)
		{
			$conn->query("update $tbl_contact set display=0,deleted='".time()."' where id = '".$tik[$i]."'");
		}
		header("location: ?act=contact_list");
	}
?>
<div class="page-title">
  <div>
    <h1><i class="fa fa-dashboard"></i> Quản lý thư liên hệ</h1>
    <p></p>
  </div>
  <div>
    <ul class="breadcrumb">
			...
			<div class="table-responsive">
				<form action="?act=contact_list" method="post" id="frm_sanpham">
					...
					<table class="table table-striped table-bordered">
						...
						<tbody>
						<?php
							$q	=$conn->query("select * from $tbl_contact where display=1 order by id desc");
