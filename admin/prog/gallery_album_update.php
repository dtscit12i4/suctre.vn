...
<?php
	...
	if ($txtMod == "add")
	{
        ...
        {
            upload_image("../uploads/gallery_album/",500,300,"album");
            ...
            ($values)
                ";
            $conn->query($sql);
            header('location: ?act=gallery_album_list&id='.$_POST['txt_cat']);      
        }
	}
	if ($txtMod == "edit")
	{
...
function get_form($getMod,$id,$note)
{
	...
	if($getMod=='edit')
	{
		$name = $get->gallery_album($id,'name');
		$info = $get->gallery_album($id,'info');
        $content = $get->gallery_album($id,'content');
        $cat = $get->gallery_album($id,'cat');
	}
	elseif($_POST)
	{
		$name = $_POST['txt_name'];
    ...
    if($getMod=='edit')
    {
        $cat_name = $get->gallery($get->gallery_album($id,'cat'),'name');
        $cat_url = "?act=gallery_list&id=".$get->gallery_album($id,'cat');
    }
    else
    {
        $cat_name = $get->gallery($_GET['cat'],'name');
