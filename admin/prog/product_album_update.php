...
<?php
	...
	if ($txtMod == "add")
	{
        ...
        {
            upload_image("../uploads/product_album/",900,600,"album");
            ...
            ($values)
                ";
            $conn->query($sql);
            header('location: ?act=product_album_list&id='.$_POST['txt_cat']);      
        }
	}
	if ($txtMod == "edit")
	{
...
function get_form($getMod,$id,$note)
{
	...
	if($getMod=='edit')
	{
		$name = $get->product_album($id,'name');
		$info = $get->product_album($id,'info');
        $content = $get->product_album($id,'content');
        $cat = $get->product_album($id,'cat');
	}
	elseif($_POST)
	{
		$name = $_POST['txt_name'];
